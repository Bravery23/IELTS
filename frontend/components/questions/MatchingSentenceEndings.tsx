...
export default function MatchingSentenceEndings({ questions }: Props) {
  const { userAnswers, setAnswer, isSubmitted } = useExamStore();

  // Options are the sentence endings to choose from
  const endingOptions = questions[0]?.options || [];
        ...
        return (
          <div key={q.id} className="animate-fade-in-up flex items-start gap-3" id={`question-${q.number}`}>
            ...
            <div className="flex-1">
              ...
              >
                <option value="">— Select sentence ending —</option>
                {endingOptions.map((opt, idx) => (
